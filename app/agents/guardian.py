...
def get_known_tactics(category: str) -> list[dict]:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT name, lever, description FROM tactics WHERE category = ?",
            (category,),
        )
...
def query_guardian(category: str, scammer_turn: str, history: list[dict]) -> str:
    ...
    lever_hint = (
        ", ".join(sorted({t["lever"] for t in known_tactics}))
        or "urgency, authority, fear, scarcity, reciprocity, liking, trust-building"
    )
    ...
    for attempt in range(2):
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                temperature=0.2 + 0.3 * attempt, max_output_tokens=250
            ),
        )
