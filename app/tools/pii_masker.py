...
PHONE_REGEX = re.compile(r'\b(?:\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b|\b\+?\d{10,13}\b')
URL_REGEX = re.compile(r'\bhttps?://[^\s/$.?#].[^\s]*|\bwww\.[^\s]+|\b[a-zA-Z0-9.-]+\.[a-zA-Z]{2,6}/[^\s]*')
ETH_REGEX = re.compile(r'\b0x[a-fA-F0-9]{40}\b')
BTC_REGEX = re.compile(r'\b[13][a-km-zA-HJ-NP-Z1-9]{25,34}\b|\bbc1[ac-hj-np-z02-9]{11,71}\b')
