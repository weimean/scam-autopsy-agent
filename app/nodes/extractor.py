...
from app.schemas import AdversarialTranscript, ClassifierOutput, TacticInfo
...
class ExtractedTactic(BaseModel):
    ...
    explanation: str = Field(
        ..., description="Plain-language explanation for a non-expert"
    )
...
async def tactic_extractor(
    ctx: Context, node_input: AdversarialTranscript
) -> list[TacticInfo]:
    """
    ...
    for t in parsed.tactics:
        save_tactic_to_db(t)
        tactic_infos.append(
            TacticInfo(name=t.name, lever=t.lever, explanation=t.explanation)
        )
