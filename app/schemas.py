...
class ClassifierOutput(BaseModel):
    ...
    red_flag_hints: list[str] = Field(
        default_factory=list,
        description="Cheap surface signals for fallback/degradation",
    )
    masked_text: str = Field(
        ..., description="Raw text with PII masked with placeholders"
    )
    detected_language: str = Field(
        default="en", description="ISO 639-1 language code detected at intake"
    )
...
class ReportOutput(BaseModel):
    ...
    warning: str = Field(
        ..., description="1-2 sentence plain-language bottom line warning"
    )
    ...
    language: str = Field(
        default="en", description="ISO 639-1 code; report is written in this language"
    )
    escalation_forecast: list[EscalationStage] = Field(
        default_factory=list,
        description="Defensive forecast of likely escalation stages",
    )
