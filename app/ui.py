...
async def analyze_message(message: str, api_key: str):
    if not message or not message.strip():
        return "Please enter a message to analyze.", "{}", f"tactics catalogued: {get_tactics_count()}"
    
    env_key = os.environ.get("GEMINI_API_KEY")
    active_key = api_key.strip() if api_key else env_key
    if not active_key:
        return "❌ Error: GEMINI_API_KEY is not set. Please provide it in the input box or set it as an env var.", "{}", f"tactics catalogued: {get_tactics_count()}"
    
    os.environ["GEMINI_API_KEY"] = active_key
    os.environ["GOOGLE_GENAI_USE_VERTEXAI"] = "False"
    
    runner = InMemoryRunner(app=adk_app)
    session = await runner.session_service.create_session(app_name=runner.app_name, user_id="ui_user")
    ...
    if not result:
        return "No report generated.", "{}", f"tactics catalogued: {get_tactics_count()}"
    ...
    for t in data.get("tactics", []):
        tactics_md += f"| `{t.get('name')}` | **{t.get('lever')}** | {t.get('explanation')} |\n"
    if not data.get("tactics"):
        tactics_md = "### Detected Tactics\n*No specific tactics detected.*\n"

    forecast_md = "### Escalation Forecast\n"
    for f in sorted(data.get("escalation_forecast", []), key=lambda x: x.get("stage", 0)):
    ...
    protect_md = "### How to Protect Yourself\n" + "\n".join([f"- {item}" for item in data.get("how_to_protect", [])])
...
"""
    return report_md, json.dumps(data, indent=2, ensure_ascii=False), f"tactics catalogued: {get_tactics_count()}"

with gr.Blocks(title="Scam Autopsy Demo UI") as demo:
    gr.Markdown("# 🔬 Scam Autopsy Demo UI")
    gr.Markdown("Analyze suspicious messages using the defensive red-team replay simulation.")
    
    with gr.Row():
        txt_input = gr.Textbox(label="Suspicious Message", placeholder="Paste the message here...", lines=5)
        txt_key = gr.Textbox(label="Gemini API Key (optional if set in env)", type="password", placeholder="AI Studio Key...")
    
    with gr.Row():
        btn_analyze = gr.Button("Analyze", variant="primary")
        txt_counter = gr.Textbox(label="Database Status", value=f"tactics catalogued: {get_tactics_count()}", interactive=False)
    ...
    btn_analyze.click(fn=analyze_message, inputs=[txt_input, txt_key], outputs=[md_output, json_output, txt_counter])
