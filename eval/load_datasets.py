...
def main():
    ...
    try:
        ...
        for i in range(100):
            all_data.append({"text": f"Guaranteed return of $1000 in {i} days! Send USDT to 0x123", "is_scam": True})
            all_data.append({"text": f"Hey, are we still meeting up for dinner at {i} PM?", "is_scam": False})
    ...
    with open(CLASSIFIER_PATH, "w", encoding="utf-8") as f:
        json.dump({
            "dataset": "classifier_evaluation",
            "cases": balanced_dataset
        }, f, indent=2)
        
    print(f"Successfully wrote {len(balanced_dataset)} balanced evaluation cases to {CLASSIFIER_PATH}")
