...
def query_tactics(category: str) -> list[dict]:
    ...
    cursor.execute("SELECT id, name, category, lever, description, example_masked FROM tactics WHERE category = ?", (category,))
    rows = cursor.fetchall()
    keys = ["id", "name", "category", "lever", "description", "example_masked"]
    return [dict(zip(keys, r)) for r in rows]

@mcp.tool()
def add_tactic(name: str, category: str, lever: str, description: str, example_masked: str) -> dict:
    ...
    try:
        ...
        return {"added": False, "reason": f"Tactic with name '{name}' and category '{category}' already exists."}
